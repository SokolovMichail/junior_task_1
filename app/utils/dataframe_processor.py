...
class DataframeProcessor:
    ...
    def construct_result_dataframe(requested_file:FileModel,db:Session):
        # Переделать на генерацию по проектам
        ...
        result_dataframe = pd.DataFrame.from_records(dataframe_prepend_list)
    ...
    def generate_product_row(db:Session, project:models.Project):
        ...
        for plan in plans:
            result[plan.date]["plan"] = plan.value
        for fact in facts:
            result[fact.date]["fact"] = fact.value
        return result
    ...
    def __generate_columns(sorted_dates: List[datetime]) -> List[str]:
        ...
        columns = list(chain.from_iterable(
            (generate_date_plan_fact_string(x,"plan"),
             generate_date_plan_fact_string(x,"fact"))
